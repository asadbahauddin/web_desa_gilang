...
$stmt = mysqli_prepare($conn, "SELECT id, foto, keterangan FROM galeri WHERE id = ? LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  hapus_file_upload($galeri['foto']);
  $stmt = mysqli_prepare($conn, "DELETE FROM galeri WHERE id = ?");
  mysqli_stmt_bind_param($stmt, 'i', $id);
  mysqli_stmt_execute($stmt);
  header('Location: /admin/galeri/index.php?deleted=1');
  exit;
}
...
<head>
  ...
  <title>Hapus Foto — Panel Admin Desa Gilang</title>
  ...
  <style>
    ...
    .danger-preview{display:flex;align-items:center;gap:16px;padding:20px 28px;border-bottom:1px solid var(--ap-line);}
    .danger-preview img{width:64px;height:64px;border-radius:10px;object-fit:cover;border:2px solid var(--ap-line);}
  ...
  <div class="admin-main">
    <header class="admin-topbar">
      <div style="display:flex;align-items:center;gap:14px;">
        ...
        <h2 class="admin-topbar__title">Hapus Foto</h2>
      </div>
      ...
      <div class="danger-card">
        <div class="danger-card__header">
          ...
          <div class="danger-card__header-text">
            <h2>Hapus Foto</h2>
            <p>Tindakan ini akan menghapus foto secara permanen dan tidak dapat dikembalikan.</p>
          </div>
        </div>

        <div class="danger-preview">
          <img src="<?php echo htmlspecialchars($galeri['foto']); ?>" alt="">
          <span><?php echo htmlspecialchars($galeri['keterangan']); ?></span>
        </div>

        <div class="danger-card__body">
          <form method="POST" action="">
            <div class="danger-actions">
              <a href="/admin/galeri/index.php" class="btn-batal">Batal</a>
              <button type="submit" class="btn-hapus">Ya, Hapus Foto</button>
            </div>
          </form>
        </div>
      </div>
