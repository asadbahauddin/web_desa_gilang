...
$stmt = mysqli_prepare($conn, "SELECT id, judul FROM pengumuman WHERE id = ? LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  mysqli_stmt_bind_param($stmt, 'i', $id);
  mysqli_stmt_execute($stmt);
  header('Location: /admin/pengumuman/index.php?deleted=1');
  exit;
}
...
<head>
  ...
  <title>Hapus Pengumuman — Panel Admin Desa Gilang</title>
  ...
  <div class="admin-main">
    <header class="admin-topbar">
      <div style="display:flex;align-items:center;gap:14px;">
        ...
        <h2 class="admin-topbar__title">Hapus Pengumuman</h2>
      </div>
      ...
      <div class="danger-card">
        <div class="danger-card__header">
          ...
          <div class="danger-card__header-text">
            ...
            <p>Tindakan ini akan menghapus pengumuman secara permanen dan tidak dapat dikembalikan.</p>
          </div>
        </div>

        <div class="danger-preview"><?php echo htmlspecialchars($pengumuman['judul']); ?></div>

        <div class="danger-card__body">
          <form method="POST" action="">
            <div class="danger-actions">
              <a href="/admin/pengumuman/index.php" class="btn-batal">Batal</a>
              <button type="submit" class="btn-hapus">Ya, Hapus Pengumuman</button>
            </div>
          </form>
        </div>
      </div>
